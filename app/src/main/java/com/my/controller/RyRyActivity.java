...
public class RyRyActivity extends AppCompatActivity {
    ...
    private XRecyclerView recylcerview;//外层recyclerview
    ...
    private void initRecyclerView() {
        recylcerview = (XRecyclerView) findViewById(R.id.recylcerview);
        //竖直排列、正向排序
        recylcerview.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));
        //添加了一个灰色背景
        recylcerview.setArrowImageView(R.mipmap.zlb_sdk_iconfont_downgrey);
        recylcerview.setRefreshProgressStyle(ProgressStyle.BallScale);
        recylcerview.setLoadingMoreProgressStyle(ProgressStyle.BallBeat);
        recylcerview.setBackgroundResource(R.color.c_e0e0e2);

        recylcerview.setLoadingListener(new XRecyclerView.LoadingListener() {
            @Override
            public void onRefresh() {
                new Handler().postDelayed(new Runnable() {
                    public void run() {
                        recylcerview.refreshComplete();
                    }
            ...
            public void onLoadMore() {
                new Handler().postDelayed(new Runnable() {
                    public void run() {
                        recylcerview.loadMoreComplete();
                    }

                }, 1000);            //refresh data here
            }
        });
        recylcerview.setRefreshing(true);
    ...
    private class RecyclerViewAdapter extends RecyclerView.Adapter<BaseHolder> {
        ...
        public void onBindViewHolder(BaseHolder holder, int position) {
            if (holder instanceof HorizontalViewHolder) {
                ...
                holder.refreshData(data.verticalData.get(position - 1), position - 1);
            }
        ...
        public int getItemCount() {
            return 1 + data.verticalData.size();
        }
